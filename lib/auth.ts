...
function config(): AuthConfig {
  ...
  if (!userId || !email || !displayName || (!passwordHash && !legacyPassword)) {
    throw new Error(
      'AUTH_USER_ID, AUTH_EMAIL, AUTH_DISPLAY_NAME e AUTH_PASSWORD_HASH são obrigatórios.',
    );
  }
...
export async function login(
  ...
  password: string,
  request: Request,
): Promise<{ ok: true; user: AuthUser; token: string } | { ok: false; status: 401 | 429 }> {
  ...
  const attempts = await db()
    .prepare(
      'SELECT COUNT(*)::int AS count FROM auth_login_attempts WHERE fingerprint = ? AND attempted_at > ?',
    )
    .bind(fingerprint, cutoff)
    .first<Row>();
  if (Number(attempts?.count ?? 0) >= MAX_ATTEMPTS) return { ok: false, status: 429 };
  // Always verify the password, even when the e-mail is unknown, so the
  // response time does not reveal which account is configured.
  const passwordIsValid = account.passwordHash
    ? await verifyPassword(password, account.passwordHash)
    : await verifyPlainPassword(password, account.legacyPassword ?? '');
  const valid = normalizedEmail === account.email && passwordIsValid;
  if (!valid) {
    await db()
      .prepare('INSERT INTO auth_login_attempts (id, fingerprint, attempted_at) VALUES (?, ?, ?)')
      .bind(id(), fingerprint, now())
      .run();
  ...
  await db().batch([
    db()
      ...
      .bind(account.userId, account.email, account.displayName, now(), now()),
    db()
      .prepare('DELETE FROM auth_login_attempts WHERE fingerprint = ? OR attempted_at < ?')
      .bind(fingerprint, cutoff),
    db()
      .prepare('DELETE FROM auth_sessions WHERE user_id = ? OR expires_at < ?')
      .bind(account.userId, now()),
    db()
      .prepare(
        'INSERT INTO auth_sessions (id, user_id, token_hash, expires_at, created_at) VALUES (?, ?, ?, ?, ?)',
      )
      .bind(id(), account.userId, tokenHash, expiresAt, now()),
  ]);
  return {
    ok: true,
    user: { userId: account.userId, email: account.email, displayName: account.displayName },
    token,
  };
}

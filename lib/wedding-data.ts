...
export async function getSnapshot(identity: WorkspaceIdentity): Promise<WeddingSnapshot> {
  ...
  return {
    ...
    guests: guestRows.map((row) => ({ id: text(row, 'id'), invitationGroupId: text(row, 'invitation_group_id') || null, fullName: text(row, 'full_name'), groupName: text(row, 'group_name'), groupType: text(row, 'group_type'), role: text(row, 'role'), ageGroup: text(row, 'age_group'), rsvp: text(row, 'rsvp'), linkUrl: text(row, 'link_url') })),
    guestInvitations,
